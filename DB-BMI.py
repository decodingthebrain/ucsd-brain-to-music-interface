...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    welcomeText = visual.TextStim(win=win, name='welcomeText',
        ...
        languageStyle='LTR',
        depth=-1.0);
    # Run 'Begin Experiment' code from set_counter
    counter = 0
    ...
    # Run 'Begin Experiment' code from counter
    counter += 1
    ...
    for thisTrial in trials:
        ...
        while continueRoutine:
            ...
            # if song_played is starting this frame...
            if song_played.status == NOT_STARTED and tThisFlip >= 5-frameTolerance:
                # keep track of start time/frame for later
                song_played.frameNStart = frameN  # exact frame index
                song_played.tStart = t  # local t and not account for scr refresh
                song_played.tStartRefresh = tThisFlipGlobal  # on global time
                # add timestamp to datafile
                thisExp.addData('song_played.started', tThisFlipGlobal)
                # update status
                song_played.status = STARTED
                song_played.play(when=win)  # sync with win flip
            
            # if song_played is stopping this frame...
            if song_played.status == STARTED:
                if bool(False) or song_played.isFinished:
                    # keep track of stop time/frame for later
                    song_played.tStop = t  # not accounting for scr refresh
                    song_played.tStopRefresh = tThisFlipGlobal  # on global time
                    song_played.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
        ...
        song_played.pause()  # ensure sound has stopped at end of Routine
        ...
        beep.seek(0)
        # create starting attributes for song_rating
        song_rating.keys = []
        song_rating.rt = []
        ...
        while continueRoutine:
            ...
            # if song_rating is starting this frame...
            if song_rating.status == NOT_STARTED and tThisFlip >= 1.0-frameTolerance:
                ...
                waitOnFlip = True
                win.callOnFlip(song_rating.clock.reset)  # t=0 on next screen flip
                win.callOnFlip(song_rating.clearEvents, eventType='keyboard')  # clear events on next screen flip
            if song_rating.status == STARTED and not waitOnFlip:
                ...
                if len(_song_rating_allKeys):
                    song_rating.keys = _song_rating_allKeys[-1].name  # just the last key pressed
        ...
        # check responses
        if song_rating.keys in ['', [], None]:  # No response was made
            song_rating.keys = None
        trials.addData('song_rating.keys',song_rating.keys)
        if song_rating.keys != None:  # we had a response
        ...
        continueRoutine = True
        # update component parameters for each repeat
        # create starting attributes for familiarity_rating
        ...
        while continueRoutine:
            ...
            # *familiarity_rating* updates
            waitOnFlip = False
            
            # if familiarity_rating is starting this frame...
            if familiarity_rating.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                # keep track of start time/frame for later
                familiarity_rating.frameNStart = frameN  # exact frame index
                familiarity_rating.tStart = t  # local t and not account for scr refresh
                familiarity_rating.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(familiarity_rating, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                ...
                # keyboard checking is just starting
                waitOnFlip = True
                win.callOnFlip(familiarity_rating.clock.reset)  # t=0 on next screen flip
                win.callOnFlip(familiarity_rating.clearEvents, eventType='keyboard')  # clear events on next screen flip
            if familiarity_rating.status == STARTED and not waitOnFlip:
                ...
                if len(_familiarity_rating_allKeys):
                    familiarity_rating.keys = _familiarity_rating_allKeys[-1].name  # just the last key pressed
        ...
        # check responses
        if familiarity_rating.keys in ['', [], None]:  # No response was made
            familiarity_rating.keys = None
        trials.addData('familiarity_rating.keys',familiarity_rating.keys)
        if familiarity_rating.keys != None:  # we had a response
